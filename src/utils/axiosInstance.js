  ...
  function (request) {
    ...
    if (request.data) {
      if (request.data.profile_image) {
        ...
        const encryptedRequest = CryptoJS.AES.encrypt(JSON.stringify(param), encrypkey).toString();
...
axiosInstance.interceptors.response.use(
 ...
 it is passed to the `.then()` or `.catch()` methods of the Promise chain. */
  function (response) {
    ...
    // console.log(response.config.url, decryptedResponse)

    if (response.data.response_data) {
      const encrypkey = process.env.ENCRYPTION_KEY || process.env.NEXT_PUBLIC_ENCRYPTION_KEY;
      const bytes = CryptoJS.AES.decrypt(response.data.response_data, encrypkey);
...
/* The code `export const axiosInstanceWithoutEncryp = axios.create({ ... })` is creating another
instance of the Axios library with a custom configuration. This instance is used for making API
requests without encryption. */
export const axiosInstanceWithoutEncryp = axios.create({
  baseURL: process.env.API_BASE_URL || process.env.NEXT_PUBLIC_API_BASE_URL,
  headers: {
    apikey: process.env.API_KEY || process.env.NEXT_PUBLIC_API_KEY,
  },
...
/* The code block `axiosInstanceWithoutEncryp.interceptors.request.use(...)` is an interceptor for
Axios requests. It is used to modify the request configuration before the request is sent. */
axiosInstanceWithoutEncryp.interceptors.request.use(
  config => {
    if (window.localStorage) {
      const token = storageInstance?.getStorage('authDataToken');
      if (token) {
